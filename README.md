## Forgetfulino 2.0.0 – never lose your sketch again
...
|-------------|-----------------|----------------|-------------------|
| **`begin()`** | Initializes the library and prints `Forgetfulino is here` once on Serial. | Call **once** in `setup()`, after `Serial.begin(...)`. | Quick check that the library is active and Serial works. |
